...
def _do_create_account(post_vars, extended_profile=None):
    ...
    profile.admission_time = str(post_vars.get('year_of_admission')) + '-' + str(post_vars.get('month_of_admission'))
...
def create_account(request, post_override=None):  # pylint: disable-msg=too-many-statements
    ...
    repath = request.META.get('HTTP_REFERER', '/').split('/')[-1]
    ...
    try:
        ...
        js['field'] = 'password'
        js['value'] = _("the two password should be the same")
        return JsonResponse(js, status=400)
